...
const Login = createWithRemoteLoader({
    ...
    const {title, registerUrl, forgetUrl, type, onSubmit} = Object.assign({}, {
        title: '登录', type: 'email', registerUrl: '', forgetUrl: ''
    }, p);
    ...
    const {Input} = fields;
    return <Form type="inner" size="large" enterSubmit onSubmit={(formData) => {
        if (rememberUser) {
        ...
        <Space className="space-full" size={10} direction="vertical">
            <SubmitButton block size="large">登录</SubmitButton>
            <Row justify="space-between">
                ...
                <Col>
                    <Button className={style['forget-button']} type="link" size="small" onClick={() => {
